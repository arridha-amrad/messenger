        ...
        sendMessage: builder.mutation<IMessage, ISendMessageRequestBody & { roomId?: string }>({
            ...
            async onQueryStarted(reqBody, { dispatch, queryFulfilled }) {
                try {
                    ...
                    dispatch(
                        chatApiSlice.util.updateQueryData('getMessages', reqBody.roomId, (draft: IMessage[]) => {
                            draft.push(body);
                        })
                    );
                    dispatch(
            ...
            transformResponse(baseQueryReturnValue: { message: IMessage }, meta, arg) {
...
export const { useSendMessageMutation, useGetRoomsQuery, useGetMessagesQuery } = chatApiSlice;
